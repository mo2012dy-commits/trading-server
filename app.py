...
# الربط مع بينانس (لجلب الأسعار والرصيد الحقيقي فقط)
...
# --- سجل الصفقات الوهمي (يُحفظ في ذاكرة السيرفر حالياً) ---
paper_trades = []
...
def dashboard():
    try:
        ...
        # حساب الربح/الخسارة للصفقات الوهمية المفتوحة
        current_paper_pnl = 0
...
def place_paper_order():
    # تنفيذ صفقة وهمية وحفظها في السجلات
    try:
        data = request.json
        status = fetch_indicators()
        ...
        return jsonify({'status': 'success', 'message': 'تم تسجيل صفقة وهمية', 'trade': new_trade})
    except Exception as e:
        return jsonify({'status': 'error', 'message': str(e)})

@app.route('/kill', methods=['POST'])
def kill_switch():
    # إغلاق جميع الصفقات في السجلات فقط
    closed_count = 0
    for trade in paper_trades:
        if trade['status'] == 'OPEN':
            trade['status'] = 'CLOSED'
